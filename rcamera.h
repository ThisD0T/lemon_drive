 ...
 *   LICENSE: zlib/libpng
 *
 *   Copyright (c) 2022-2023 Christoph Wagner (@Crydsch) & Ramon Santamaria
 *(@raysan5)
 *
...
void UpdateCamera(Camera *camera, int mode) {
  ...
  bool lockView = ((mode == CAMERA_FIRST_PERSON) ||
                   (mode == CAMERA_THIRD_PERSON) || (mode == CAMERA_ORBITAL));
  ...
  if (mode == CAMERA_ORBITAL) {
    ...
    if (IsKeyDown(KEY_DOWN))
      CameraPitch(camera, -CAMERA_ROTATION_SPEED, lockView, rotateAroundTarget,
                  rotateUp);
    if (IsKeyDown(KEY_UP))
      CameraPitch(camera, CAMERA_ROTATION_SPEED, lockView, rotateAroundTarget,
                  rotateUp);
    ...
    // if (IsKeyDown(KEY_LEFT_CONTROL)) CameraMoveUp(camera,
    // -CAMERA_MOVE_SPEED);
  }
